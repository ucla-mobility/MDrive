...
build_rpc_shim() {
  local repo_root="$1"
  local src="$repo_root/tools/close_ebadf_suppress.c"
  local out="$repo_root/tools/close_ebadf_suppress.so"

  [[ -f "$src" ]] || die "Missing RPC shim source: $src (incomplete clone?)"

  if [[ -f "$out" && "$out" -nt "$src" ]]; then
    log "RPC shim already built and up-to-date: $out"
    return
  fi

  log "Building RPC shim from $src"
  gcc -O2 -shared -fPIC -o "$out" "$src" -ldl \
    || die "Failed to compile RPC shim ($src)"
  log "Built RPC shim: $out"
}

patch_carlaue4_sh() {
  local carla_dir="$1"
  local repo_root="$2"
  local sh_path="$carla_dir/CarlaUE4.sh"
  local shim_abs="$repo_root/tools/close_ebadf_suppress.so"

  [[ -f "$sh_path" ]] || die "CarlaUE4.sh not found: $sh_path"

  if grep -q 'carla_rpc_fix injection' "$sh_path"; then
    log "CarlaUE4.sh already injects RPC shim."
    return
  fi

  python3 - "$sh_path" "$shim_abs" <<'PY'
import pathlib
import sys

sh_path = pathlib.Path(sys.argv[1])
shim_abs = sys.argv[2]

lines = sh_path.read_text(encoding="utf-8").splitlines(keepends=True)

block = (
    "\n"
    "# carla_rpc_fix injection - applies CARLA 0.9.12 RPC double-close patches\n"
    "# via LD_PRELOAD shim. Regenerated by download_and_setup_carla.sh after\n"
    "# each tarball extract. See carla_patches/README.md.\n"
    f"_RPC_SHIM=\"{shim_abs}\"\n"
    "if [ -f \"$_RPC_SHIM\" ]; then\n"
    "    export LD_PRELOAD=\"$_RPC_SHIM${LD_PRELOAD:+:$LD_PRELOAD}\"\n"
    "fi\n"
    "\n"
)
...
inserted = False
for line in lines:
    if not inserted and 'CarlaUE4-Linux-Shipping" CarlaUE4' in line:
        out.append(block)
        inserted = True
    out.append(line)
...
if not inserted:
    raise SystemExit(
        "Could not find CarlaUE4-Linux-Shipping launch line in CarlaUE4.sh"
    )

sh_path.write_text("".join(out), encoding="utf-8")
PY
  log "Patched CarlaUE4.sh to inject RPC shim via LD_PRELOAD."
}
...
run_validation() {
  ...
  # Verify the RPC shim activated. The shim emits "[carla_rpc_fix]" lines at
  # __attribute__((constructor)) time, before the engine prints its banner.
  # -a forces text mode (server log may contain bytes that look binary).
  if ! grep -aq '\[carla_rpc_fix\]' "$server_log"; then
    die "RPC shim did not load via LD_PRELOAD. Check tools/close_ebadf_suppress.so and CarlaUE4.sh. See: $server_log"
  fi
  local rpc_ok rpc_skip
  rpc_ok="$(grep -acE '\[carla_rpc_fix\] (Fix [234]: OK|Fix 1.*: active)' "$server_log" || true)"
  rpc_skip="$(grep -acE '\[carla_rpc_fix\] Fix [0-9]+: SKIP' "$server_log" || true)"
  if (( rpc_skip > 0 )); then
    warn "RPC shim reported $rpc_skip SKIP line(s) — CARLA binary may not match patch targets. See: $server_log"
  fi
  if (( rpc_ok < 4 )); then
    die "RPC shim activated but only $rpc_ok/4 fixes reported success. See: $server_log"
  fi
  log "RPC shim verified: all 4 fixes active."
...
main() {
  ...
  # RPC crash patches: build LD_PRELOAD shim and inject it into the freshly
  # extracted CarlaUE4.sh. Both steps are idempotent. See carla_patches/README.md.
  build_rpc_shim "$repo_root"
  patch_carlaue4_sh "$carla_dir" "$repo_root"
